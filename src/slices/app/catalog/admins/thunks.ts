...
export const startSavingAdmin = (data: any): ThunkAction<void, RootState, unknown, Action<string>> => {
...
export const startUpdateAdmin = (data: any, id: number): ThunkAction<void, RootState, unknown, Action<string>> => {
    return withLoadingOverlay ( async ( dispatch: ThunkDispatch<RootState, unknown, Action<string>> ) => {
        try {
...
export const startDeleteAdmin = (id: number): ThunkAction<void, RootState, unknown, Action<string>> => async (dispatch: ThunkDispatch<RootState, unknown, Action<string>>) => {
    return withLoadingOverlay ( async ( dispatch: ThunkDispatch<RootState, unknown, Action<string>> ) => {
        try {
            await api.delete(`admin/administrador/${id}`, null);
            dispatch( startLoadingAdmins() );
